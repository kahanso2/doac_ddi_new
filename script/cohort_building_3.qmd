...
blank_oac_ndc <- id_blank_GENNME_added |> 
  mutate(oac_flag = str_detect(generic_name, str_c(oac_drug_list, collapse = "|"))) |> 
...
get_ndc_by_drug_name <- function() {
  redbook_ndcs <- redbook |> 
    filter(str_detect(GENNME, str_c(oac_drug_list, collapse = "|"))) |> 
    distinct(NDCNUM) |> 
    pull()
  
  # Combine redbook NDCs with those from blank_oac_ndc vector above
  all_ndcs <- unique(c(as.character(redbook_ndcs), as.character(blank_oac_ndc)))
...
ccaed_2009_2021 <- extract_drug_name_data(
  dataset_path = "//pharm-c-psop/TruvenData/Truven Data R/ccae/d", 
  output_path = "C:/Users/kahanso2/OneDrive - University of Illinois Chicago/Documents_Backup_Microsoft/doac_ddi_new/data/ccaed_2009_2021.parquet", 
  enrolid_filter = NULL,
  ndc_filter = oac_ndc
)

mdcrd_2009_2021 <- extract_drug_name_data(
  dataset_path = "//pharm-c-psop/TruvenData/Truven Data R/mdcr/d", 
  output_path = "C:/Users/kahanso2/OneDrive - University of Illinois Chicago/Documents_Backup_Microsoft/doac_ddi_new/data/mdcrd_2009_2021.parquet", 
  enrolid_filter = NULL,
  ndc_filter = oac_ndc)

# Bind drug files from CCAE and MDCR
all_drug <- bind_rows(ccaed_2009_2021, mdcrd_2009_2021) # 29,015,802 observations

normalize_oac_name <- function(x) {
  x0 <- x %>%
    # lower for matching
    str_to_lower() %>%
    # drop bracketed brand names: [Coumadin], etc.
    str_remove_all("\\[.*?\\]") %>%
    # remove strengths like "7.5 mg", "1 mg", "10 mg", "5mg", "5 mg/mL"
    str_remove_all("\\b\\d+(?:\\.\\d+)?\\s*(?:mg|mcg|g|mg/ml|mg\\/ml|ml)\\b") %>%
    # remove dosage form / route words that sneak into labels
    ...
    # remove separators / duplicates like "rivaroxaban;rivaroxaban"
    str_replace_all("[;:]+", " ") %>%
    # squeeze spaces
    str_squish()
...
canon_oac <- function(x) {
  ...
  dplyr::case_when(
    ...
    stringr::str_detect(x0, "dabigatran")                            ~ "Dabigatran",  # covers “dabigatran etexilate (mesylate)”
    stringr::str_detect(x0, "warfarin")                              ~ "Warfarin",    # covers “warfarin sodium”
...
all_drug_cleaned <- all_drug |>
  filter(!is.na(ENROLID)) |>
  left_join(id_blank_GENNME_added, by = "NDCNUM") |>
  mutate(GENNME = if_else(is.na(GENNME), generic_name, GENNME)) |>
  select(-generic_name) |>
  mutate(GENNME = canon_oac(GENNME))


# Save dataset so don't have to do that again
all_drug_cleaned |> write_parquet("C:/Users/kahanso2/OneDrive - University of Illinois Chicago/Documents_Backup_Microsoft/doac_ddi_new/data/all_drug_2009_2021.parquet")

#### CODE TO OPEN DATASET WITHOUT RUNNING ABOVE ####

# Open dataset
all_drug_2009_2021 <- read_parquet("C:/Users/kahanso2/OneDrive - University of Illinois Chicago/Documents_Backup_Microsoft/doac_ddi_new/data/all_drug_2009_2021.parquet")

# Save unique IDs of oac users
all_oac_users <- unique(all_drug_2009_2021$ENROLID) # 2,360,604 users
...
```{r}
options(duckplyr_prudence = "lavish")
...
write_parquet(cleaned_drug_data, "C:/Users/kahanso2/Documents/doac-ddi/data/cleaned_drug_data.parquet")
...
# Save dataset so don't have to do that again
all_oac_index |> write_parquet("C:/Users/kahanso2/Documents/doac-ddi/data/all_oac_index.parquet")

# Open dataset
all_oac_index <- read_parquet("C:/Users/kahanso2/Documents/doac-ddi/data/all_oac_index.parquet")
...
all_enroll |> write_parquet("C:/Users/kahanso2/Documents/doac-ddi/data_control/enrollment_parquet.parquet") 

# Read parquet file back into the environment
enrollment_parquet <- read_parquet("C:/Users/kahanso2/Documents/doac-ddi/data_control/enrollment_parquet.parquet") 
...
write_parquet(all_oac_index_ce, "C:/Users/kahanso2/Documents/doac-ddi/data/all_oac_index_ce.parquet")

# Reopen if needed
all_oac_index_ce <- read_parquet("C:/Users/kahanso2/Documents/doac-ddi/data/all_oac_index_ce.parquet")
...
write_parquet(oac_cohort, "C:/Users/kahanso2/Documents/doac-ddi/data/oac_cohort.parquet")
oac_cohort <- read_parquet("C:/Users/kahanso2/Documents/doac-ddi/data/oac_cohort.parquet")
...
write_rds(analytic_cohort_oac, "C:/Users/kahanso2/Documents/doac-ddi/data/analytic_cohort_oac.rds")

analytic_cohort_oac <- read_rds("C:/Users/kahanso2/Documents/doac-ddi/data/analytic_cohort_oac.rds")
...
oac_precipitant_processing <- function(oac){
  ...
  drug_mapping <- readxl::read_excel("drug_mapping.xlsx")
...
# Apixaban
write_rds(dataset_for_loop_apixaban,            "dataset_for_loop_apixaban.rds")
write_rds(dataset_for_loop_outcome_apixaban,    "dataset_for_loop_outcome_apixaban.rds")
write_rds(apixaban_cohort,                      "apixaban_cohort.rds")
write_rds(apixaban_vector,                      "apixaban_vector.rds")

# Rivaroxaban
write_rds(dataset_for_loop_rivaroxaban,         "dataset_for_loop_rivaroxaban.rds")
write_rds(dataset_for_loop_outcome_rivaroxaban, "dataset_for_loop_outcome_rivaroxaban.rds")
write_rds(rivaroxaban_cohort,                   "rivaroxaban_cohort.rds")
write_rds(rivaroxaban_vector,                   "rivaroxaban_vector.rds")

# Dabigatran
write_rds(dataset_for_loop_dabigatran,          "dataset_for_loop_dabigatran.rds")
write_rds(dataset_for_loop_outcome_dabigatran,  "dataset_for_loop_outcome_dabigatran.rds")
write_rds(dabigatran_cohort,                    "dabigatran_cohort.rds")
write_rds(dabigatran_vector,                    "dabigatran_vector.rds")

# Warfarin
write_rds(dataset_for_loop_warfarin,            "dataset_for_loop_warfarin.rds")
write_rds(dataset_for_loop_outcome_warfarin,    "dataset_for_loop_outcome_warfarin.rds")
write_rds(warfarin_cohort,                      "warfarin_cohort.rds")
write_rds(warfarin_vector,                      "warfarin_vector.rds")
...
# Apixaban
dataset_for_loop_apixaban         <- read_rds("dataset_for_loop_apixaban.rds")
dataset_for_loop_outcome_apixaban <- read_rds("dataset_for_loop_outcome_apixaban.rds")
apixaban_cohort                   <- read_rds("apixaban_cohort.rds")
apixaban_vector                   <- read_rds("apixaban_vector.rds")

# Rivaroxaban
dataset_for_loop_rivaroxaban         <- read_rds("dataset_for_loop_rivaroxaban.rds")
dataset_for_loop_outcome_rivaroxaban <- read_rds("dataset_for_loop_outcome_rivaroxaban.rds")
rivaroxaban_cohort                   <- read_rds("rivaroxaban_cohort.rds")
rivaroxaban_vector                   <- read_rds("rivaroxaban_vector.rds")

# Dabigatran
dataset_for_loop_dabigatran         <- read_rds("dataset_for_loop_dabigatran.rds")
dataset_for_loop_outcome_dabigatran <- read_rds("dataset_for_loop_outcome_dabigatran.rds")
dabigatran_cohort                   <- read_rds("dabigatran_cohort.rds")
dabigatran_vector                   <- read_rds("dabigatran_vector.rds")

# Warfarin
dataset_for_loop_warfarin         <- read_rds("dataset_for_loop_warfarin.rds")
dataset_for_loop_outcome_warfarin <- read_rds("dataset_for_loop_outcome_warfarin.rds")
warfarin_cohort                   <- read_rds("warfarin_cohort.rds")
warfarin_vector                   <- read_rds("warfarin_vector.rds")
...
data.table::fwrite(res_unadj, "sccs_results_unadjusted.csv")
saveRDS(res_unadj, "sccs_results_unadjusted.rds")
...
data.table::fwrite(res_adj, "sccs_results_adjusted.csv")
saveRDS(res_adj, "sccs_results_adjusted.rds")
rm(res_adj); gc()
...
# Newest Shrinkage (Need to check vs. below - dev 1029/2025)
...
# ---------- 0) Input & optional exclusions ----------
dt <- as.data.table(res_adj)

# keep only valid estimates
dt <- dt[is.finite(Estimate) & is.finite(SE) & SE > 0]
dt[, var_beta := SE^2]

# Optional: drop if var_beta > 10 (as in Zhou)
dt <- dt[var_beta <= 10]

# Optional: if you have counts of cases ever exposed, enforce >=5
if ("n_exposed_cases" %in% names(dt)) {
  dt <- dt[is.finite(n_exposed_cases) & n_exposed_cases >= 5]
}

# ---------- 1) Shrink function (fixed prior) ----------
shrink_fixed <- function(d, prior_var_log = 0.25, prior_mean_log = 0) {
  # Normal-Normal update per-pair
  se2   <- d[["SE"]]^2
  tau2  <- prior_var_log
  mu0   <- prior_mean_log
...
# ---------- 2) Apply primary (0.25) and secondary (0.67) priors ----------
shrunk_025 <- shrink_fixed(copy(dt), prior_var_log = 0.25)  # primary analysis
shrunk_067 <- shrink_fixed(copy(dt), prior_var_log = 0.67)  # secondary analysis

# ---------- 3) OPTIONAL: Ratio of RRs vs Pravastatin (delta method) ----------
# Requires that `object == "Pravastatin"` results are present with the same `drug` naming.
# log ratio = logRR_OAC - logRR_Prava; Var = Var_OAC + Var_Prava; CI by usual normal approx
ratio_vs_prava <- function(d) {
  if (!("Pravastatin" %in% d$object)) return(NULL)

  # take one OAC at a time against Pravastatin by precipitant (drug)
...
ratio_025 <- ratio_vs_prava(shrunk_025)
ratio_067 <- ratio_vs_prava(shrunk_067)

# ---------- 4) Quick counts per object ----------
...
# ---------- 5) Save compact outputs ----------
dir.create("shrinkage_outputs", showWarnings = FALSE)
...
data.table::fwrite(shrunk_025[, intersect(keep_cols, names(shrunk_025)), with=FALSE],
                   "shrinkage_outputs/adjusted_shrunk_prior025.csv")
saveRDS(shrunk_025[, intersect(keep_cols, names(shrunk_025)), with=FALSE],
        "shrinkage_outputs/adjusted_shrunk_prior025.rds")

data.table::fwrite(shrunk_067[, intersect(keep_cols, names(shrunk_067)), with=FALSE],
                   "shrinkage_outputs/adjusted_shrunk_prior067.csv")
saveRDS(shrunk_067[, intersect(keep_cols, names(shrunk_067)), with=FALSE],
        "shrinkage_outputs/adjusted_shrunk_prior067.rds")

if (!is.null(ratio_025)) {
  data.table::fwrite(ratio_025, "shrinkage_outputs/ratio_vs_pravastatin_prior025.csv")
  saveRDS(ratio_025,          "shrinkage_outputs/ratio_vs_pravastatin_prior025.rds")
}
if (!is.null(ratio_067)) {
  data.table::fwrite(ratio_067, "shrinkage_outputs/ratio_vs_pravastatin_prior067.csv")
  saveRDS(ratio_067,          "shrinkage_outputs/ratio_vs_pravastatin_prior067.rds")
}

cat("Semi-Bayes shrinkage complete. Files in shrinkage_outputs/ \n")


# Load this up in the morning and inspect. restarting R to clear memory now. 
```
