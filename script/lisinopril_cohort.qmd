...
# Apply continuous exposure - data.table
```{r}
...
compute_ratio_vs_lisinopril <- function(label,
  ...
  if (!all(needed %in% names(oac_dt))) stop("OAC table missing needed columns: ", paste(setdiff(needed, names(oac_dt)), collapse = ", "))
  if (!all(needed %in% names(nc_dt)))  stop("Control table missing needed columns: ", paste(setdiff(needed, names(nc_dt)), collapse = ", "))

  # ---- 3) Get negative-control (Lisinopril) rows by drug ----
  ...
  out_list <- lapply(objects, function(obj) {
    ...
    m[, `:=`(
      ...
      se_log_ratio  = sqrt(var_o + var_nc),
      ratio_rr      = exp(log_rr_o - log_rr_nc),
      ratio_lower   = exp((log_rr_o - log_rr_nc) - 1.96 * sqrt(var_o + var_nc)),
      ratio_upper   = exp((log_rr_o - log_rr_nc) + 1.96 * sqrt(var_o + var_nc)),
      ratio_z       = (log_rr_o - log_rr_nc) / sqrt(var_o + var_nc),
      ratio_p       = 2 * pnorm(abs((log_rr_o - log_rr_nc) / sqrt(var_o + var_nc)), lower.tail = FALSE),
      ratio_sig_any = (exp((log_rr_o - log_rr_nc) - 1.96 * sqrt(var_o + var_nc)) > 1 |
                       exp((log_rr_o - log_rr_nc) + 1.96 * sqrt(var_o + var_nc)) < 1)
    )]
  ...
  data.table::fwrite(ratio_dt, out_csv)
