...
ccaed_neg_con_2009_2021 <- extract_drug_name_data(
  ...
  output_path = file.path(proj_root, "data", "control", "ccaed_neg_con_2009_2021.parquet"), 
  enrolid_filter = NULL,
...
mdcrd_neg_con_2009_2021 <- extract_drug_name_data(
  ...
  output_path = file.path(proj_root, "data", "control", "mdcrd_neg_con_2009_2021.parquet"), 
  enrolid_filter = NULL,
  ndc_filter = neg_con_ndc)
...
all_drug_neg_con_2009_2021 |> write_parquet(file.path(proj_root, "data", "control", "all_drug_neg_con_2009_2021.parquet"))
...
# ── Save dataset so we don't have to redo ───────────────────────────────────────
write_parquet(all_neg_con_index, file.path(proj_root, "data", "control", "all_neg_con_index.parquet"))

# Open dataset later without rerunning cleaning ────────────────────
# all_neg_con_index <- read_parquet(file.path(proj_root, "data", "control", "all_neg_con_index.parquet"))
...
write_parquet(continuous_enrollment_result, file.path(proj_root, "data", "control", "continuous_enrollment_result.parquet")) 

# Read in files
# read_parquet(file.path(proj_root, "data", "control", "continuous_enrollment_result.parquet")) 
# ids_with_ce <- unique(continuous_enrollment_result$ENROLID)
...
write_parquet(all_enroll, file.path(proj_root, "data", "control", "enrollment_parquet.parquet")) 

# Read parquet file back into the environment
enrollment_parquet <- read_parquet(file.path(proj_root, "data", "control", "enrollment_parquet.parquet")) 
...
write_parquet(all_neg_con_index_ce, file.path(proj_root, "data", "control", "all_neg_con_index_ce.parquet"))

# Reopen if needed
# all_neg_con_index_ce <- read_parquet(file.path(proj_root, "data", "control", "all_neg_con_index_ce.parquet"))
...
write_parquet(
  neg_con_cohort,
  file.path(proj_root, "data", "control", "neg_con_cohort.parquet")
)

# neg_con_cohort <- read_parquet(file.path(proj_root, "data", "control", "neg_con_cohort.parquet"))
...
ccae_bleed_outcome_no_trauma <- identify_bleed_outcome(
  ...
  output_path_event = "C:/Users/kahanso2/OneDrive - University of Illinois Chicago/Documents_Backup_Microsoft/doac_ddi_new/data/control/ccae_bleed_outcome_no_trauma.parquet"
)
...
mdcr_bleed_outcome_no_trauma <- identify_bleed_outcome(
  ...
  output_path_event = "C:/Users/kahanso2/OneDrive - University of Illinois Chicago/Documents_Backup_Microsoft/doac_ddi_new/data/control/mdcr_bleed_outcome_no_trauma.parquet"
)
...
all_outcome |> write_parquet("C:/Users/kahanso2/OneDrive - University of Illinois Chicago/Documents_Backup_Microsoft/doac_ddi_new/data/control/all_inpatient_bleed_no_trauma.parquet")
...
all_inpatient_bleed_no_trauma <- read_parquet("C:/Users/kahanso2/OneDrive - University of Illinois Chicago/Documents_Backup_Microsoft/doac_ddi_new/data/control/all_inpatient_bleed_no_trauma.parquet")
...
write_rds(analytic_cohort_neg_con, "C:/Users/kahanso2/OneDrive - University of Illinois Chicago/Documents_Backup_Microsoft/doac_ddi_new/data/control/analytic_cohort_neg_con.rds")
...
saveRDS(
  oac_spans_negcon,
  file.path(proj_root, "data/control/loop_datasets/oac_spans_negcon.rds")
)
```
...
# Lisinopril
write_rds(dataset_for_loop_neg_con, file.path(proj_root, "/data/control/loop_datasets/dataset_for_loop_neg_con.rds"))
write_rds(dataset_for_loop_outcome_neg_con, file.path(proj_root, "/data/control/loop_datasets/dataset_for_loop_outcome_neg_con.rds"))
write_rds(neg_con_cohort, file.path(proj_root, "/data/control/loop_datasets/neg_con_cohort.rds"))
write_rds(neg_con_vector, file.path(proj_root, "/data/control/loop_datasets/neg_con_vector.rds"))
...
# Negative Control
dataset_for_loop_neg_con         <- read_rds(file.path(proj_root, "/data/control/loop_datasets/dataset_for_loop_neg_con.rds"))
dataset_for_loop_outcome_neg_con <- read_rds(file.path(proj_root, "/data/control/loop_datasets/dataset_for_loop_outcome_neg_con.rds"))
neg_con_cohort                   <- read_rds(file.path(proj_root, "/data/control/loop_datasets/neg_con_cohort.rds"))
neg_con_vector                   <- read_rds(file.path(proj_root, "/data/control/loop_datasets/neg_con_vector.rds"))
oac_spans_negcon                 <- readRDS(file.path(proj_root, "data/control/loop_datasets/oac_spans_negcon.rds")) # Need to load to drop OAC days
...
data.table::fwrite(res_unadj, file.path(proj_root, "results/control/sccs_results_unadjusted.csv"))
saveRDS(res_unadj, file.path(proj_root, "results/control/sccs_results_unadjusted.rds"))
...
data.table::fwrite(res_adj, file.path(proj_root, "results/control/sccs_results_adjusted.csv"))
saveRDS(res_adj, file.path(proj_root, "results/control/sccs_results_adjusted.rds"))
...
res_unadj <- readRDS(file.path(proj_root, "results/control/sccs_results_unadjusted.rds"))
...
res_adj <- readRDS(file.path(proj_root, "results/control/sccs_results_adjusted.rds"))
...
dir.create(file.path(proj_root, "results",  "control"), showWarnings = FALSE, recursive = TRUE)
...
run_and_save_shrinkage <- function(result_df, label, save_ratios = TRUE) {
  ...
  save_both <- function(dt2, stem) {
    out_csv <- file.path(proj_root, "results",  "control", paste0(stem, ".csv"))
    out_rds <- file.path(proj_root, "results",  "control", paste0(stem, ".rds"))
  ...
  if (isTRUE(save_ratios)) {
    ...
    if (!is.null(r025)) {
      data.table::fwrite(r025, file.path(proj_root, "results",  "control", paste0("ratio_vs_pravastatin_", label, "_prior025.csv")))
      saveRDS(r025,              file.path(proj_root, "results",  "control", paste0("ratio_vs_pravastatin_", label, "_prior025.rds")))
    }
    if (!is.null(r067)) {
      data.table::fwrite(r067, file.path(proj_root, "results",  "control", paste0("ratio_vs_pravastatin_", label, "_prior067.csv")))
      saveRDS(r067,              file.path(proj_root, "results",  "control", paste0("ratio_vs_pravastatin_", label, "_prior067.rds")))
    }
  }
}
...
cat("\nSemi-Bayes shrinkage complete for available datasets. Files saved under: ",
    file.path(proj_root, "results",  "control"), "\n")
```
...
compute_ratio_vs_lisinopril <- function(label,
  ...
  # ---- 1) Build file paths ----
  oac_path <- file.path(proj_root, "results",
                        paste0(label, "_shrunk_", prior_suffix, ".rds"))
  nc_path  <- file.path(proj_root, "results", "control",
                        paste0(label, "_shrunk_", prior_suffix, ".rds"))
  ...
  out_csv <- file.path(proj_root, "results", "ratio_of_ratios", paste0(stem, ".csv"))
  out_rds <- file.path(proj_root, "results", "ratio_of_ratios", paste0(stem, ".rds"))
...
build_ddi_screen_table <- function(label = "unadjusted",
  ...
  nc_path  <- file.path(proj_root, "results", "control",
                        paste0(label, "_shrunk_", prior_suffix, ".rds"))
  ror_path <- file.path(proj_root, "results", "ratio_of_ratios",
  ...
  out_csv <- file.path(proj_root, "results", "ratio_of_ratios",
                       paste0("ddi_screen_table_", label, "_", prior_suffix, ".csv"))
  out_rds <- file.path(proj_root, "results", "ratio_of_ratios",
                       paste0("ddi_screen_table_", label, "_", prior_suffix, ".rds"))
  
  dir.create(file.path(proj_root, "results", "ratio_of_ratios"), showWarnings = FALSE, recursive = TRUE)
...
```{r}
# ============================================
# Build Zhou-style Tables 2 and 3
#   - Uses:
#       proj_root/results/unadjusted_shrunk_prior025.rds
#       proj_root/results/ratio_of_ratios/ddi_screen_table_unadjusted_prior025.rds
#   - Produces:
#       proj_root/results/tables/table2_oac_screen_summary_unadj_prior025.{csv,rds}
#       proj_root/results/tables/table3_ddi_candidates_unadj_prior025.{csv,rds}
# ============================================
...
# ---- 1) Paths ----
oac_shrunk_path <- file.path(proj_root, "results", "unadjusted_shrunk_prior025.rds")
ddi_path        <- file.path(proj_root, "results", "ratio_of_ratios",
                             "ddi_screen_table_unadjusted_prior025.rds")

if (!file.exists(oac_shrunk_path)) {
  stop("OAC shrunk file not found: ", oac_shrunk_path)
}

# ---- 2) Ensure DDI table exists (build if needed) ----
if (!file.exists(ddi_path)) {
  if (!exists("build_ddi_screen_table")) {
    stop(
      "DDI screen table not found at:\n  ", ddi_path,
      "\n and build_ddi_screen_table() is not in the environment."
    )
  }
  message("DDI screen table not found; creating via build_ddi_screen_table()...")
  ddi_unadj_025 <- build_ddi_screen_table(
    label       = "unadjusted",
    prior_suffix = "prior025",
    proj_root   = proj_root,
...
# Read the DDI table (or reuse if already in memory)
ddi_results <- if (exists("ddi_unadj_025")) {
  ddi_unadj_025
} else {
  readRDS(ddi_path)
}
ddi_results <- as.data.table(ddi_results)

# Read OAC shrunk results
oac_shrunk <- as.data.table(readRDS(oac_shrunk_path))

# ---- 3) Table 2A: overall OAC screen summary (by OAC object) ----
#   - Number of precipitants
#   - Number with an estimate
#   - Number with significant shrunk RR
table2_overall <- oac_shrunk[
  is.finite(shrunken_irr),
  .(
    n_precipitants  = uniqueN(drug),
    n_with_estimate = sum(!is.na(shrunken_irr)),
    n_sig_shrunk    = sum(sig_any %in% TRUE)
  ),
  by = object
][order(object)]

# ---- 4) Table 2B: summary restricted to OAC hits + RoR (based on ddi_results) ----
table2_hits <- ddi_results[
  ,
  .(
    n_precipitants_sig_oac = uniqueN(drug),
    n_sig_ror              = sum(ratio_sig_any %in% TRUE)
  ),
  by = object
][order(object)]

# Merge into a single Table 2
table2 <- merge(table2_overall, table2_hits, by = "object", all.x = TRUE)
setorder(table2, object)

# ---- 5) Table 3: detailed DDI candidates (Zhou-style) ----
# You can uncomment the filter(ratio_sig_any == TRUE) line if you want to
# restrict to RoR-significant hits only.
table3 <- ddi_results %>%
  # Optionally restrict to ratio-of-ratios significant:
  # filter(ratio_sig_any == TRUE) %>%
  arrange(object, desc(irr_oac)) %>%
  mutate(
    ...
    # Placeholder for manual annotation of DDI mechanism
...
# ---- 6) Save tables ----
out_dir <- file.path(proj_root, "results", "tables")
dir.create(out_dir, showWarnings = FALSE, recursive = TRUE)

# Table 2
fwrite(table2, file.path(out_dir, "table2_oac_screen_summary_unadj_prior025.csv"))
saveRDS(table2, file.path(out_dir, "table2_oac_screen_summary_unadj_prior025.rds"))

# Table 3
fwrite(table3, file.path(out_dir, "table3_ddi_candidates_unadj_prior025.csv"))
saveRDS(table3, file.path(out_dir, "table3_ddi_candidates_unadj_prior025.rds"))

message("Table 2 saved to: ", file.path(out_dir, "table2_oac_screen_summary_unadj_prior025.{csv,rds}"))
message("Table 3 saved to: ", file.path(out_dir, "table3_ddi_candidates_unadj_prior025.{csv,rds}"))
...
```{r}
# ============================================
# Build Zhou-style Tables 2 & 3 as GT tables
#   – Uses:
#       proj_root/results/sccs_results_unadjusted.rds
#       proj_root/results/unadjusted_shrunk_prior025.rds
#       proj_root/results/ratio_of_ratios/ddi_screen_table_unadjusted_prior025.rds
#       proj_root/data/loop_datasets/*_vector.rds
#   – Produces:
#       proj_root/results/tables/table2_zhou_style_unadj_prior025.{csv,rds,html}
#       proj_root/results/tables/table3_ddi_candidates_unadj_prior025.{csv,rds,html}
# ============================================
...
# ---- 1) Paths ----
res_unadj_path   <- file.path(proj_root, "results", "sccs_results_unadjusted.rds")
shrunk_path      <- file.path(proj_root, "results", "unadjusted_shrunk_prior025.rds")
ddi_path         <- file.path(proj_root, "results", "ratio_of_ratios",
                              "ddi_screen_table_unadjusted_prior025.rds")

vec_dir          <- file.path(proj_root, "data", "loop_datasets")
vec_paths <- list(
...
# sanity checks
missing_files <- c(
  res_unadj_path[!file.exists(res_unadj_path)],
  shrunk_path[!file.exists(shrunk_path)],
  ddi_path[!file.exists(ddi_path)],
  vec_paths[!file.exists(unlist(vec_paths))]
)
...
oac_shrunk  <- as.data.table(readRDS(shrunk_path))
ddi_results <- as.data.table(readRDS(ddi_path))

# Ensure expected columns exist in ddi_results
needed_ddi_cols <- c("object", "drug",
                     "irr_oac", "lower_oac", "upper_oac",
                     "irr_nc",  "lower_nc",  "upper_nc",
                     "ratio_rr","ratio_lower","ratio_upper",
                     "ratio_sig_any", "ddi_pattern")
missing_ddi_cols <- setdiff(needed_ddi_cols, names(ddi_results))
if (length(missing_ddi_cols) > 0) {
  stop("ddi_results is missing columns: ",
       paste(missing_ddi_cols, collapse = ", "))
}

# Load precipitant vectors (pre-loop "identified" precipitants)
precip_vectors <- lapply(vec_paths, readRDS)
names(precip_vectors) <- names(vec_paths)

# ---- 3) Helper: clean numeric ranges as 'a.b–c.d' ----
...
# ---- 4) Build Table 2 (Zhou-style summary) ----

# 4a. # precipitants with RR estimate from SCCS (any non-NA Estimate)
rr_summary <- res_unadj[is.finite(Estimate),
                        .(n_with_rr = uniqueN(drug)),
                        by = object]

# 4b. # included in semi-Bayes (after var_beta / n_exposed filters)
sb_summary <- oac_shrunk[
  is.finite(shrunken_irr),
  .(
...
# 4c. # precipitants identified (pre-loop vectors)
...
# 4d. RoR summary from DDI screen
ror_summary <- ddi_results[
  ...
  .(
    ...
    n_sig_ror     = sum(ratio_sig_any %in% TRUE),
    # Simple flag for "potential DDI signal":
    #  - OAC semi-Bayes RR > 1
    #  - RoR > 1 and significant
    n_potential_ddi = sum(
...
# 4e. Merge pieces into one Table 2 dataset
table2_dt <- id_summary %>%
  full_join(rr_summary,  by = "object") %>%
  full_join(sb_summary,  by = "object") %>%
  full_join(ror_summary, by = "object") %>%
  arrange(object)

# Nicely labeled / ordered columns
table2_dt <- table2_dt %>%
  mutate(
    object = factor(object,
                    levels = c("Apixaban","Rivaroxaban","Dabigatran","Warfarin"))
  ) %>%
  arrange(object) %>%
...
# ---- 5) Build Table 3 (detailed DDI candidates) ----
# Here we keep all rows in ddi_results (you can filter ratio_sig_any == TRUE if desired)

table3_dt <- ddi_results %>%
  arrange(object, desc(irr_oac)) %>%
  mutate(
    irr_oac_ci = sprintf("%.2f (%.2f, %.2f)", irr_oac, lower_oac, upper_oac),
    irr_nc_ci  = ifelse(
    ...
    # Placeholder for manual mechanism annotation (edit as needed)
    mechanism  = ifelse("mechanism" %in% names(ddi_results),
                        as.character(mechanism),
                        NA_character_)
  ) %>%
  select(
    object,
...
# ---- 6) Save raw tables (CSV/RDS) ----
out_dir <- file.path(proj_root, "results", "tables")
dir.create(out_dir, showWarnings = FALSE, recursive = TRUE)

# Table 2
fwrite(table2_dt, file.path(out_dir, "table2_zhou_style_unadj_prior025.csv"))
saveRDS(table2_dt, file.path(out_dir, "table2_zhou_style_unadj_prior025.rds"))

# Table 3
fwrite(table3_dt, file.path(out_dir, "table3_ddi_candidates_unadj_prior025.csv"))
saveRDS(table3_dt, file.path(out_dir, "table3_ddi_candidates_unadj_prior025.rds"))

# ---- 7) Publication-style GT tables ----
...
table2_gt <- table2_dt %>%
  ...
  fmt_number(
    columns = c(n_identified, n_with_rr, n_in_sb,
                n_sig_sb, n_for_ror, n_sig_ror, n_potential_ddi),
    decimals = 0
  ) %>%
  tab_options(
    table.font.size = 11,
  ...
  tab_source_note(
    source_note = "*Potential DDI signals defined here as precipitants with semi-Bayes RR > 1, " %>%
      paste0("ratio of RRs > 1, and statistically significant ratio of RRs; ",
             "this rule can be modified in the code as needed.")
  )

gtsave(
  table2_gt,
  filename = file.path(out_dir, "table2_zhou_style_unadj_prior025.html")
)
...
gtsave(
  table3_gt,
  filename = file.path(out_dir, "table3_ddi_candidates_unadj_prior025.html")
)

message("Table 2 written to:\n  ",
        file.path(out_dir, "table2_zhou_style_unadj_prior025.{csv,rds,html}"))
message("Table 3 written to:\n  ",
        file.path(out_dir, "table3_ddi_candidates_unadj_prior025.{csv,rds,html}"))
...
pub_table
gtsave(pub_table, "C:/Users/kahanso2/OneDrive - University of Illinois Chicago/Documents_Backup_Microsoft/doac_ddi_new/results/tables/table1.html")
